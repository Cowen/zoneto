...
def test_query_comps_s37_columns_present_when_enriched(tmp_path: Path) -> None:
    """Given: Enriched parquet includes s37_monetary_value + s37_benefit_text.
    When: query_comps is called.
    Then: Both S.37 columns appear in result records."""
    import datetime
    ...
    pl.DataFrame(
        {
            ...
            "lat": [43.650],
            "lon": [-79.380],
            "dev_approved": pl.Series([1], dtype=pl.Int8),
            "dev_appealed": pl.Series([0], dtype=pl.Int8),
            "dev_days_to_decision": pl.Series([365], dtype=pl.Int32),
            "proposed_storeys": pl.Series([10], dtype=pl.Int32),
            "proposed_units": pl.Series([100], dtype=pl.Int32),
            "description": ["OZ application"],
            "street_num": ["100"],
            "street_name": ["King St"],
            "s37_monetary_value": pl.Series([500000.0], dtype=pl.Float64),
            "s37_benefit_text": pl.Series(["Cash; Parkland"], dtype=pl.String),
        }
    ).write_parquet(path)

    results = query_comps(path, years=5)
    assert len(results) == 1
    rec = results[0]
    assert "s37_monetary_value" in rec
    assert rec["s37_monetary_value"] == 500000.0
    assert "s37_benefit_text" in rec
    assert rec["s37_benefit_text"] == "Cash; Parkland"


def test_query_comps_s37_columns_null_when_absent(tmp_path: Path) -> None:
    """Given: Enriched parquet does not have S.37 columns.
    When: query_comps is called.
    Then: s37_monetary_value and s37_benefit_text are null in results."""
    import datetime
    ...
    assert len(results) == 1
    rec = results[0]
    assert rec.get("s37_monetary_value") is None
    assert rec.get("s37_benefit_text") is None
