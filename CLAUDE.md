...
<!-- Last reviewed against: main (BERT description similarity + programmatic confidence cap) -->
...
- `compute_bert_embeddings(data_dir)` -- encodes every dev_application
  `description` with `BAAI/bge-small-en-v1.5` (384-dim) and writes two parallel
  files to `data/enriched/`: `desc_bert_embeddings.npy` (float32 array of shape
  `[n_rows, 384]`) and `desc_bert_index.parquet` (metadata columns:
  `folderrsn`, `application_type`, plus `dev_approved`, `dev_appealed`,
  `zoning_class` when present). Used by `score_description_similarity_bert`
  for higher-quality semantic similarity than TF-IDF+SVD. Idempotent — re-runs
  cheaply because sentence-transformers caches the model.
...
- Lifespan loads the bylaw index (`data/bylaw_index/`) into `app.state.bylaw_index`
  when `chunks.parquet` exists; otherwise sets it to `None`.
- **BERT model loading** (`app.state.bert_model`): when
  `data/enriched/desc_bert_embeddings.npy` exists, lifespan ensures a
  `SentenceTransformer` is loaded. If `bylaw_index` is already loaded, it
  reuses `bylaw_index.model` (same BAAI/bge-small-en-v1.5 weights); otherwise
  it loads `SentenceTransformer("BAAI/bge-small-en-v1.5")` standalone. When
  embeddings are absent, `bert_model` is `None` and `/evaluate` falls back to
  TF-IDF+SVD similarity.
- LLM client (`app.state.llm_client`): instantiated via `make_llm_client()`
  when `ANTHROPIC_API_KEY` is set; otherwise `None`.
...
Two scorers compute cosine similarity between a project description and the
enriched dev_applications corpus. The BERT scorer is preferred; the TF-IDF+SVD
scorer is the fallback when BERT embeddings are absent.

#### TF-IDF + SVD scorer (fallback)

- `score_description_similarity(description, data_dir, model_dir, *, top_n=20, min_similarity=0.1, zoning_class=None) -> dict[str, Any] | None`
...
#### BERT scorer (preferred)

- `score_description_similarity_bert(description, data_dir, *, model, top_n=20, min_similarity=0.1, zoning_class=None) -> dict[str, Any] | None`
- Loads pre-computed embeddings from `data/enriched/desc_bert_embeddings.npy`
  (float32, shape `[n_rows, 384]`) and the parallel metadata index
  `data/enriched/desc_bert_index.parquet`. Generated by
  `enrich.compute_bert_embeddings()` using `BAAI/bge-small-en-v1.5`.
- `model` is a `SentenceTransformer` instance supplied by the caller (loaded
  once at app startup — see App Factory). The scorer encodes the query
  description via `model.encode([...])` and computes cosine similarity against
  the pre-computed corpus.
- Returns `None` when `model is None` or either embeddings/index file is
  missing, or on any internal error.

#### Shared return shape

On success both scorers return:
`{"top_matches": list[dict], "appeal_rate": float | None, "approval_rate": float | None, "n_similar": int}`.

Each match contains `similarity` (rounded to 3 d.p.) plus whichever metadata
columns are available (TF-IDF: `folderrsn`, `application_type`, `street_address`,
`dev_appealed`, `dev_approved`; BERT: all columns in `desc_bert_index.parquet`,
which currently includes `folderrsn`, `application_type`, `dev_approved`,
`dev_appealed`, `zoning_class` when present in enriched data).

`appeal_rate` is the share of labelled top matches that were appealed (None
when no labelled matches). `approval_rate` is returned for API compatibility
but is NOT surfaced in the narrator — `dev_approved` labels only cover ~9.6%
of OZ/SA applications, creating survivorship bias.

When `zoning_class` is supplied, the result also includes:
- `zone_matched_n_similar` (int) — count of same-zone applications above the
  similarity threshold
- `zone_matched_approval_rate` (float | None) — same-zone approval rate
  (returned but, like `approval_rate`, not surfaced in the narrator)
- `zone_matched_appeal_rate` (float | None) — appeal rate for same-zone
  comparables; this is the cleanest zone-specific adversarial-risk signal
  (no survivorship bias) and IS surfaced in the narrator.

The deprecated `zone_base_approval_rate` / `zone_base_n` fields have been
removed — they reported the corpus-wide zone base rate, which carries the same
survivorship bias as the overall approval rate.
...
- The system prompt uses a step-by-step confidence assignment procedure:
  Step 1: extreme violation check (any ≥3× zone limit → cap 10-30);
  Step 2: base band (70-80 for zero violations + compatible zone use,
  55-65 for mismatched use, 35-55 for moderate violations);
  Step 3: adjust within band ±8 for MTSA/secondary plan/zoning exception (up)
  or high appeal rate/heritage district (down). The Step 2 floor is absolute —
  appeal rate and data gaps cannot push below it.
- **Programmatic floor (deterministic, applied after LLM parse):** when
  `violations == []` and `site.permitted_use_category` contains "mixed" or
  "commercial residential" (case-insensitive), the parsed score is raised to
  `max(score, 70)`. Enforces Step 2A independently of LLM temperature.
- **Programmatic cap (deterministic, applied after LLM parse):** when ANY of
  `extracted.proposed_storeys / site.zoning_max_storeys` or
  `extracted.proposed_units / site.zoning_max_units` is ≥ 3.0, the parsed
  score is clamped to `min(score, 30)`. Enforces Step 1 (extreme violation
  cap) deterministically — no LLM override possible regardless of comparable
  appeal rate, comparable approvals, or any other signal.
- `_format_description_similarity(sim, *, site_zoning_class=None)` gates the
  best-comparable outcome line (Council-approved / not-approved) on whether
  the best match's `zoning_class` equals `site_zoning_class` (or either is
  None). Cross-zone comparables show zone only, no outcome — preventing
  cross-zone outcome bleed-through. Zone-matched appeal rate (when present)
...
- When `description_similarity` is provided (the dict returned by
  ...
  similar OZ/SA applications, the closest comparable's zone (for cross-zone
  relevance), `appeal_rate` (overall), and `zone_matched_appeal_rate` (same-zone
  comparables when available). Approval rates are intentionally excluded —
  `dev_approved` labels cover only ~9.6% of applications (survivorship bias).
  The section is omitted when the dict is None or `n_similar == 0`.
- LLM `max_tokens` is 1200 for evaluation narration (400 for follow-up questions
...
**Similarity backend selection:** when `request.app.state.bert_model` is set
(i.e. BERT embeddings exist on disk and the SentenceTransformer was loaded at
startup), the endpoint calls `score_description_similarity_bert` for higher
semantic accuracy. Otherwise it falls back to `score_description_similarity`
(TF-IDF + SVD). Both pass `zoning_class=site.get("zoning_class")` so the
narrator receives zone-matched stats.
